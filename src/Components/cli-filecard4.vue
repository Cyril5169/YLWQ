...
<div class="wrapper-jun">
    <div class="div1">
        <cliDepute v-show="showcliDepute" @dispearDepute="dispearDepute()" @changeCheck="getWeiTuo" :addCheck="codeCheck" :only="only"></cliDepute>
        <div class="ways">经营方式</div>
        <form class="ways-contents">
            <input type="radio"  v-model="pickWay" value="has-has" id="has-has" name="bussiness-way" :disabled='only'><label for="has-has">&nbsp;&nbsp;有营业执照-有对账公号&nbsp;&nbsp;&nbsp;（请仅填公司汇款账号信息）</label><br/>
            <input type="radio" :disabled="faPiaoDisable||only" v-model="pickWay"  value="has-no" id="has-no" name="bussiness-way"><label for="has-no">&nbsp;&nbsp;有营业执照-无对账公号&nbsp;&nbsp;&nbsp;（请仅填个人汇款账号信息）</label><br/>
            ...
            <input type="radio" :disabled="faPiaoDisable||only" v-model="pickWay"  value="no-no" id="no-no" name="bussiness-way"><label for="no-no">&nbsp;&nbsp;无营业执照&nbsp;&nbsp;&nbsp;（请仅填个人汇款账号信息）</label><br/>
        </form>
        <div class="gray-word">*请选择经营类型后填写对应的账号</div>
        <div class="bankwrapper">
            <div class="company">
                <div class="account-info">公司汇款账号信息</div>
                <div><span>汇款人</span><input type="text" v-model="cardobj.account1Name" class="readonly" readonly :disabled = "isDisable" ></div>
                <div><span>汇款银行</span><input type="text" v-model="account1Bank" :disabled = "isDisable" :readOnly='only' :class="{readonly:only}"></div>
                <div><span>账号</span><input type="text" v-model="account1" :disabled = "isDisable" :readOnly='only' :class="{readonly:only}"></div> 
                <!-- <div><span>银行所在地</span><v-distpicker @selected="onSelected1" :disabled = "isDisable" class="address"></v-distpicker></div> -->
                <div class="loc-wrapper">
                    <span>银行所在地</span><select :disabled="isDisable||only" v-model="province1">
                        <option value="" disabled selected>{{dataProvince1}}</option>
                        <option v-for="(item,index) in provinces1" :value="item" :key="index">{{ item.regionName }}</option>
                    </select>
                    <select :disabled="isDisable||only" v-model="city1">
                        <option value="" disabled selected>{{dataCity1}}</option>
                        <option v-for="(item,index) in citys1" :value="item" :key="index">{{ item.regionName }}</option>
                    </select>
                    <select :disabled="isDisable||only" v-model="area1">
                        <option value="" disabled selected>{{dataArea1}}</option>
                        <option v-for="(item,index) in areas1" :value="item" :key="index">{{ item.regionName }}</option>
                    </select>
                </div>
            ...
            <div class="personal">
                ...
                <div><span>汇款人</span><input type="text" :disabled = "!isDisable" class="readonly" readonly :value="cardobj.account2Name"></div>
                <div><span>汇款银行</span><input type="text" :disabled = "!isDisable" v-model="account2Bank"  :readOnly='only' :class="{readonly:only}"></div>
                <div><span>账号</span><input type="text" :disabled = "!isDisable" v-model="account2" :readOnly='only' :class="{readonly:only}"></div> 
                <!-- <div><span>银行所在地</span><v-distpicker :disabled = "!isDisable" class="address"></v-distpicker></div> -->
                <div class="loc-wrapper">
                    <span>银行所在地</span><select :disabled="!isDisable||only" v-model="province2" >
                        <option value="" disabled selected>{{dataProvince2}}</option>                        
                        <option v-for="(item,index) in provinces2" :value="item" :key="index">{{ item.regionName }}</option>
                    </select>
                    <select :disabled="!isDisable||only" v-model="city2">
                        <option value="" disabled selected>{{dataCity2}}</option>                        
                        <option v-for="(item,index) in citys2" :value="item" :key="index">{{ item.regionName }}</option>
                    </select>
                    <select :disabled="!isDisable||only" v-model="area2">
                        <option value="" disabled selected>{{dataArea2}}</option>                        
                        <option v-for="(item,index) in areas2" :value="item" :key="index">{{ item.regionName }}</option>
                    </select>
                </div>
            </div>
        </div>
        <div class="gray-word">*请先选择账号实体对应的类型，然后再逐个选择待上传资质文件类型<span class="photo-tip">（点击图片可查看大图）</span></div>   
        ...
        <div class="photo-wrapper-right">
            ...
            <a href="javascript:;" class="a-upload"><input type="file" accept="image/jpg,image/jpeg,image/png" :disabled='only' name="photo1" id="" @change="changepic" >上传文件</a>
            <h1>{{ photoType }}</h1>
            <div>{{operateMessage}}</div>
            <div class="number-wrapper" v-show="cardShow">身份证号：<input type="text" name="IDcard" class="IDcard" v-model="file1IdcardNo" :readOnly='only' :class="{readonly:only}"></div>
            <div class="number-wrapper" v-show="yingyeShow">营业执照注册号：<input type="text" name="IDcard" class="IDcard" v-model="file2BusinesslicenseNo" :readOnly='only' :class="{readonly:only}">
                <div>营业执照有效期：<el-date-picker class="file2BusinesslicenseEnd" v-model="file2BusinesslicenseEnd" value-format="yyyy-MM-dd" type="date" placeholder="选择日期" :disabled="noEnd || (this.only == true)"></el-date-picker> <input type="checkbox" id="longdate" v-model="noEnd" :disabled='only' @click="noEnd=!noEnd;file2BusinesslicenseEnd=''"><label for="longdate">长期有效</label></div>
            </div>
            <div class="number-wrapper" v-show="suiwuShow">税务证号：<input type="text" v-model="file4GtqcNo" name="IDcard" class="IDcard" :readOnly='only' :class="{readonly:only}"></div>            
        </div>
    </div>
    <div class="button">
...
export default {
    ...
    data() {
        return {
            ...
            allphotoTypesList:['1-身份证头像面','2-身份证国徽面','3-营业执照','4-税务登记证（国税）'],
   ...
   //点击小图看大图
    showPhoto(){
            ...
            window.open(newP,'newWin','width=667,height=885,top='+t+',left='+l+',toolbar=no,menubar=no,location=no,status=yes');
    },
    ...
    changepic(e) {
            ...
            if(picavalue.size/1024 >500) {  //如果图片大于500K则执行以下程序
                reader.onloadend = function() {
                    ...
                    img.onload = function(){
                        ...
                        console.log('压缩后上传');
                        self.operateMessage =  new Date().getFullYear() + "-" + (new Date().getMonth() + 1) + "-" + new Date().getDate() + " " + new Date().getHours() + ":" + (new Date().getMinutes() >= 10? new Date().getMinutes(): ("0" + new Date().getMinutes())) + " " + self.cardobj.cid;
                        ...
                        fd.append("fileName",self.cardobj.cid + "-(" + self.$store.state.year + ")-" + temp[self.showImageIndex]);
                        ...
                        .then(res => {
                            if(res.data != null && res.data.code == 0){
                                let url = self.filesarr[self.showImageIndex].url = remoteImageURL + res.data.data.path +"?t=" + Math.random().toFixed(4);
                                self.filesarr[self.showImageIndex].posturl = res.data.data.path;
                                document.getElementById('show').src= url+"?t="+Math.random().toFixed(4);
        ...
        save(){
            ...
            this.$emit('mergeMessage',{
                ...
                file2BusinesslicenseEnd:((typeof(this.file2BusinesslicenseEnd)) != 'string' && this.file2BusinesslicenseEnd != null)?this.file2BusinesslicenseEnd.getFullYear() + "-" + (this.file2BusinesslicenseEnd.getMonth() + 1) + "-" + this.file2BusinesslicenseEnd.getDate():this.file2BusinesslicenseEnd,
        ...
        check(){
            ...
            if(this.recordTitle == "资料卡通过" || this.recordTitle == "业务员审核中" || this.recordTitle == "订单部审核中"){
            ...
            if(this.accountType == "公司：三证五证合一" || this.accountType == "个体户：未取得税务登记证"){
                //3张图片的时候
            ...
            }else if(this.accountType == "公司：三证五证未合一" || this.accountType == "个体户：已取得税务登记证"){
                //4张图片的时候
                if(!this.filesarr[2].posturl){
                    this.$alert("请上传营业执照");
                    return false;
    ...
    watch:{
        ...
        province1(val,oldval){
            this.$axios.post('/yulan/areaRegion/getCity.do',this.province1)
            .then((res)=>{
        ...
        city1(val,oldval){
            this.$axios.post('/yulan/areaRegion/getCountry.do',this.city1)
            .then((res)=>{
        ...
        province2(val,oldval){
            this.$axios.post('/yulan/areaRegion/getCity.do',this.province2)
            .then((res)=>{
        ...
        city2(val,oldval){
            this.$axios.post('/yulan/areaRegion/getCountry.do',this.city2)
            .then((res)=>{
        ...
        photoType(){
            switch(this.photoType){
                case '1-身份证头像面':this.yingyeShow = false;this.cardShow = true;this.suiwuShow = false;break;
                case '2-身份证国徽面':this.yingyeShow = false;this.cardShow = false;this.suiwuShow = false;break;
                case '3-营业执照':this.yingyeShow = true;this.cardShow = false;this.suiwuShow = false;break;
                case '4-税务登记证（国税）':this.yingyeShow = false;this.cardShow = false;this.suiwuShow = true;break;
            }
            document.getElementById('show').src = this.filesarr[this.showImageIndex].url;
        ...
        accountType(){
            ...
            switch(this.accountType){
                case '公司：三证五证合一':this.photoTypesList = this.allphotoTypesList.slice(0,3);break;
                case '公司：三证五证未合一':this.photoTypesList = this.allphotoTypesList;break;
                case '个体户：已取得税务登记证':this.photoTypesList = this.allphotoTypesList;break;
                case '个体户：未取得税务登记证':this.photoTypesList = this.allphotoTypesList.slice(0,3);break;
                case '个人':this.photoTypesList = this.allphotoTypesList.slice(0,2);break;
            }
        },
        cardobj(){
            //初始化显示省市县，并将从对象上拿到的银行所在地信息拿下来赋初始值（对不起臣妾做不到啊！！！！）
            if(this.only == true){
                if(!!this.cardobj.account1Location && (this.cardobj.busientType == '公司') && (this.cardobj.hasPublicAccount == "Y")){
                ...
                if(!!this.cardobj.account2Location && ((this.cardobj.busientType == '个体户') || (this.cardobj.busientType == "公司" && this.cardobj.hasPublicAccount == "N"))){
            ...
            // //图片区域的数据初始化
            let arr = ['公司：三证五证合一','公司：三证五证未合一','个体户：已取得税务登记证','个体户：未取得税务登记证','个人'];
            if(this.cardobj.customerentitytypex != "0" && this.cardobj.customerentitytypex != null)this.accountType = arr[this.cardobj.customerentitytypex - 1];
            ...
            if(this.noEnd == false)this.file2BusinesslicenseEnd = this.cardobj.file2BusinesslicenseEnd;   //不为长期有效才赋值上去
            this.file4GtqcNo = this.cardobj.file4GtqcNo;

            if(this.cardobj.file1Idcard != "" && this.cardobj.file1Idcard != null){
                document.getElementById('show').src= remoteImageURL + this.cardobj.file1Idcard + "?t=" + Math.random().toFixed(4);
            }
            try{
                ...
                this.filesarr[0].url = remoteImageURL + this.cardobj.file1Idcard + "?t=" + Math.random().toFixed(4);
                this.filesarr[1].url = remoteImageURL + this.cardobj.file5IdcardBg + "?t=" + Math.random().toFixed(4);
                this.filesarr[2].url = remoteImageURL + this.cardobj.file2Businesslicense + "?t=" + Math.random().toFixed(4);
                this.filesarr[3].url = remoteImageURL + this.cardobj.file4Gtqc + "?t=" + Math.random().toFixed(4);        
            }catch(err){
    ...
    computed:{
        ...
        customerentitytypex(){
            switch(this.accountType){
                case '公司：三证五证合一':this.photoTypesList = this.allphotoTypesList.slice(0,3);return 1;
                case '公司：三证五证未合一':this.photoTypesList = this.allphotoTypesList;return 2;
                case '个体户：已取得税务登记证':this.photoTypesList = this.allphotoTypesList;return 3;
                case '个体户：未取得税务登记证':this.photoTypesList = this.allphotoTypesList.slice(0,3);return 4;
                case '个人':this.photoTypesList = this.allphotoTypesList.slice(0,2);return 5;
            }
        },
        hasPublicAccount(){
        ...
        account1Location(){
            if(!this.city1.regionName)
            return this.province1.regionName;
            else if(!this.area1.regionName)
            return this.province1.regionName + '-' + this.city1.regionName;
            else return this.province1.regionName + '-' + this.city1.regionName + '-' + this.area1.regionName;
        },
        account2Location(){
            if(!this.city2.regionName)
            return this.province2.regionName;
            else if(!this.area2.regionName)
            return this.province2.regionName + '-' + this.city2.regionName;
            else return this.province2.regionName + '-' + this.city2.regionName + '-' + this.area2.regionName;
        },
        //根据store来更新
        accountTypesList(){
            // console.log("列表刷新了");
            if(this.$store.state.faPiao == "增值税专用发票"){
                if(this.accountType == "个体户：已取得税务登记证" || this.accountType == "个体户：未取得税务登记证" || this.accountType == "个人"){
            ...
            if(this.$store.state.faPiao == "不开发票"){
                if(this.pickWay == "no-no"){
                    ...
                    if(this.accountType == "个人")this.accountType = "公司：三证五证合一";
                    return ['公司：三证五证合一','公司：三证五证未合一','个体户：已取得税务登记证','个体户：未取得税务登记证'];
                }    
            }
            if(this.$store.state.faPiao == "增值税普通发票"||this.$store.state.faPiao == "电子普通发票"){
                if(this.pickWay == "no-no"){
                    this.accountType = "个人";
                    return ['个人'];
                }    
                else {
                    if(this.accountType == "个人"||this.accountType == '个体户：未取得税务登记证')this.accountType = "公司：三证五证合一";
                    return ['公司：三证五证合一','公司：三证五证未合一','个体户：已取得税务登记证'];
                }                    
            }
