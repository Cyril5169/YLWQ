   ...
   <div class="page">
       <div class="content">
           <div class="filesLeft">
               ...
               <div class="intention">
                      <p class="title" style="position:relative;top:-14px">意向品牌<span class="must-input">（必选）</span></p>
                      <div class="inteninput">
                        <input type="checkbox" name="brand" id="WallPaper" v-model="databool1" :disabled='only'><label for="WallPaper">玉兰（墙纸类产品）</label><br/>
                        <input type="checkbox" name="brand" id="SoftProducts" v-model="databool2" :disabled='only'><label for="SoftProducts">玉兰·兰居尚品（软装类产品）</label>
                      </div>
               </div>
               
               <div class="nowBrand">
                   <p class="title">主营品牌（非玉兰）<span class="must-input">（必填）</span></p>
                   <input type="text" name="brandName" v-model="currentProduct" :readOnly='only' :class="{readOnly:only}">
               </div>
               <div class="receipt">
                   <p class="title" style="position:relative;top:-22px;">发票开具类型<br/><span class="must-input">（必选）</span></p>
                   <div class="recinput">
                   <input type="radio" name="receiptType" value="不开发票" id="no" v-model="invoiceType" :disabled='only || (this.cardobj.vatInvoiceFlag == "Y")'><label for="no">不开发票</label><br/>
                   <input type="radio" name="receiptType" value="增值税普通发票" id="normal" v-model="invoiceType" :disabled='only || (this.cardobj.vatInvoiceFlag == "Y")'><label for="normal">增值税普通发票</label>
                   <input type="radio" name="receiptType" value="电子普通发票" id="electron" v-model="invoiceType" :disabled='only || (this.cardobj.vatInvoiceFlag == "Y")'><label for="electron">电子普通发票</label><br/>
                   <input type="radio" name="receiptType" value="增值税专用发票" id="only" v-model="invoiceType" :disabled='only || (this.cardobj.vatInvoiceFlag == "Y")'><label for="only">增值税专用发票</label>
                   </div>
               </div>
              <div class="taxPayer">
                   <p class="title">一般纳税人</p>
                   <input type="radio" name="payerType" value="yes" id="yes" v-model="taxPayerType" :disabled='only || (this.cardobj.vatInvoiceFlag == "Y")'><label for="yes">是</label>
                   <input type="radio" name="payerType" value="not" id="not" v-model="taxPayerType" :disabled='only || (this.cardobj.vatInvoiceFlag == "Y")'><label for="not">否</label>
               </div>
            <!--发票开具类型不同选项-->
            <div class="message1" v-show="isShow1">
                 <p class="title">接收邮箱</p>
                   <input type="text" id="recipeTargetMb" name="email1"  v-model="recipeTargetMb" :readOnly='only' :class="{readOnly:only}" >
            </div>
            ...
            <div class="address">
                   <p class="title">地址</p>
                   <input type="text" name="invAddress" v-model="invAddress" :disabled="(this.cardobj.vatInvoiceFlag == 'Y')" :readOnly='only' :class="{readOnly:only}">
            </div>
            <div class="telephone">
                   <p class="title">电话</p>
                   <input type="text" name="invTelephone" v-model="invTelephone" :disabled="(this.cardobj.vatInvoiceFlag == 'Y')" :readOnly='only' :class="{readOnly:only}">
            </div>
            <div class="bank">
                   <p class="title">开户银行</p>
                   <input type="text" name="invBank" v-model="invBankname" :disabled="(this.cardobj.vatInvoiceFlag == 'Y')" :readOnly='only' :class="{readOnly:only}">
            </div>
            <div class="bankAccount">
                   <p class="title">银行账号</p>
                   <input type="text" name="invBanmaccount" v-model="invBanmaccount" :disabled="(this.cardobj.vatInvoiceFlag == 'Y')" :readOnly='only' :class="{readOnly:only}">
            </div>
           </div>
           ...
           <div class="filesRight">
              ...
              <div class="owner">
                   <p class="title">法人手机<span class="must-input">（必填）</span></p>
               ...
               <div class="owner">
                   ...
                   <input type="number" name="space" v-model="shopArea" :readOnly='only' :class="{readOnly:only}"  onpaste="return false;" oninput = "value=value.replace(/[^\d]/g,'')">
