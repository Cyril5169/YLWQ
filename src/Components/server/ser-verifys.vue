<template>
    <div class="verifys-wrapper" ref="VerifysLoad">
            ...
            <div class="filesLeft">
                ...
                <div class="intention">
                        ...
                        <div class="inteninput">
                            <input type="checkbox" disabled="disabled" name="brand" id="WallPaper" v-model="databool1"><label for="WallPaper">玉兰（墙纸类产品）</label><br/>
                            <input type="checkbox" disabled="disabled" name="brand" id="SoftProducts" v-model="databool2"><label for="SoftProducts">玉兰·兰居尚品（软装类产品）</label>
                        </div>
                </div>
                ...
                <div class="taxPayer">
                    ...
                    <input type="radio" disabled name="payerType" value="yes" id="yes" v-model="taxPayerType"><label for="yes">是</label>
                    <input type="radio" disabled name="payerType" value="not" id="not" v-model="taxPayerType"><label for="not">否</label>
                </div>                
        ...
        <div class="wrapper-jun">
            <div class="div1">
                <serDepute v-show="showcliDepute" :ccid="this.cardobj.cid" @dispearDepute="dispearDepute()" @showDeputePhoto="showPhoto" style="position:absolute;top:100px;"></serDepute>


                <div class="ways">经营方式</div>
                <form class="ways-contents">
                    <input disabled type="radio"  v-model="pickWay" value="has-has" id="has-has" name="bussiness-way"><label for="has-has">&nbsp;&nbsp;有营业执照-有对账公号&nbsp;&nbsp;&nbsp;（请仅填公司汇款账号信息）</label><br/>
                    <input disabled type="radio" v-model="pickWay"  value="has-no" id="has-no" name="bussiness-way"><label for="has-no">&nbsp;&nbsp;有营业执照-无对账公号&nbsp;&nbsp;&nbsp;（请仅填个人汇款账号信息）</label><br/>
                    <div class="add-account" v-show="isShow">
                        <a class="alert-block" @click="moreMessageInput">查看授权委托账号信息及附件</a>
                    </div>
                    <input disabled type="radio" v-model="pickWay"  value="no-no" id="no-no" name="bussiness-way"><label for="no-no">&nbsp;&nbsp;无营业执照&nbsp;&nbsp;&nbsp;（请仅填个人汇款账号信息）</label><br/>
                </form>

                <div class="bankwrapper">
                    <div class="company" v-show="cardobj.hasPublicAccount == 'Y'">
                        <div class="account-info">公司汇款账号信息</div>
                        <div><span>汇款人</span><input type="text" v-model="cardobj.account1Name" class="readOnly" readOnly></div>
                        <div><span>汇款银行</span><input type="text" v-model="cardobj.account1Bank" class="readOnly" readOnly></div>
                        <div><span>账号</span><input type="text" v-model="cardobj.account1" class="readOnly" readOnly></div> 
                        <!-- <div><span>银行所在地</span><input type="text" v-model="cardobj.account1Location" readOnly class="readOnly"></div> -->
                        <div><span>银行所在地</span><div class="longAddress" style="display:inline-block;padding:0 10px;vertical-align:top">{{cardobj.account1Location}}</div></div>
                    </div>
                    <div class="company" v-show="cardobj.hasPublicAccount == 'N'">
                        <div class="account-info">个人汇款信息</div>
                        <div><span>汇款人</span><input type="text" class="readOnly" readOnly v-model="cardobj.account2Name"></div>
                        <div><span>汇款银行</span><input type="text" class="readOnly" readOnly v-model="cardobj.account2Bank"></div>
                        <div><span>账号</span><input type="text" class="readOnly" readOnly v-model="cardobj.account2"></div> 
                        <!-- <div><span>银行所在地</span><input type="text" v-model="cardobj.account2Location" readOnly class="readOnly"></div> -->
                        <div><span>银行所在地</span><div class="longAddress" style="display:inline-block;padding:0 10px;vertical-align:top">{{cardobj.account2Location}}</div></div>
                    </div>
                </div>
                <div>
                    <p>账号实体对应的类型</p>
                ...
                <!-- <div class="photo-wrapper-right">
                    ...
                    <a href="javascript:;" class="a-upload"><input type="file" accept="image/jpg,image/jpeg,image/png" name="photo1" id="" @change="changepic">修改文件</a>
                </div> -->


                <!-- 要全部直接跳网页了。。。 -->
                <!-- <div class="checkPhoto" v-show="showBP">
                    <i class="el-icon-close" @click="showBP = !showBP"></i>
                    <img :src="bigRouter" >
                </div> -->
                <div class="gray-word">*点击图片查看大图</div>
                    ...
                    <!-- <viewer :images="images">
                      <div class="photos-infos">
                       <img v-for="src in; images" :src="src" :key="src" width="300" class= "photo-show"> 
                      </div>
                   </viewer> -->
                ...
                <!-- <div class="photos-infos">
                    <div class="photos-info">
                        <img preview="0"  class= "photo-show" id="show" alt="" :src= "file1Idcard" >
                        <div class="photo-title">1-身份证头像面</div>
                        <div class="photo-message">{{cardobj.file1Op}}</div>
                        <div class="info-wrapper">身份证号：<input type="text" name="IDcard" class="IDcard readOnly" v-model="cardobj.file1IdcardNo" readOnly></div>
                    </div>
                    <div class="photos-info">
                        <img  preview="0" class= "photo-show" id="show" alt="" :src= "file5IdcardBg">
                        <div class="photo-title">2-身份证国徽面</div>
                        <div class="photo-message">{{cardobj.file5Op}}</div>
                    </div>
                    <div class="photos-info" v-show="card3Show">
                        <img preview="0" class= "photo-show" id="show" alt="" :src= "file2Businesslicense">
                        <div class="photo-title">3-营业执照</div>
                        <div class="photo-message">{{cardobj.file2Op}}</div>
                        <div class="info-wrapper">营业执照注册号：<input type="text" name="IDcard" class="IDcard readOnly" readOnly v-model="cardobj.file2BusinesslicenseNo"></div>
                        <div class="info-wrapper">营业执照有效期：<input type="text" class="IDcard readOnly" readOnly :value="cardobj.file2BusinesslicenseNoend == 0?cardobj.file2BusinesslicenseEnd:'长期有效'"></div>
                    </div>
                    <div class="photos-info" v-show="card4Show">
                        <img preview="0" class= "photo-show" id="show" alt="" :src= "file4Gtqc" >
                        <div class="photo-title">4-税务登记证（国税）</div>
                        <div class="photo-message">{{cardobj.file4Op}}</div>
                        <div class="info-wrapper">税务证号：<input type="text" v-model="cardobj.file4GtqcNo" name="IDcard" class="IDcard readOnly" readOnly></div>            
                    </div>
                </div> -->
...
export default {
    ...
    data(){
        return {
            ...
            accountTypesList:['公司：三证五证合一','公司：三证五证未合一','个体户：已取得税务登记证','个体户：未取得税务登记证','个人'],
            accountType:"",
    ...
    methods:{
        ...
        tuihui(){
           if(this.reason == ''){
            ...
            let position = this.$store.state.user.pos[0].position;
            if(position == 'BILLDEP_APPROVE'||position == 'SALEMAN_M'||position =="SALEMAN_S"){
                this.$emit('close',false)
                this.$axios.post('/yulan/infoState/bussinessCheckCustomerInfoCard.do',{
                    "cid":this.cid,
    ...
    watch:{
        ...
        cardobj(newV){
            ...
            //4---图片赋值
            if(newV.file1Idcard)this.file1Idcard = remoteImageURL + newV.file1Idcard;
            if(newV.file2Businesslicense)this.file2Businesslicense = remoteImageURL + newV.file2Businesslicense;
            if(newV.file4Gtqc)this.file4Gtqc = remoteImageURL + newV.file4Gtqc;
            if(newV.file5IdcardBg)this.file5IdcardBg = remoteImageURL + newV.file5IdcardBg;
            loadingInstance.close();
        },
        //发票类型
        invoiceType() {
            if(this.invoiceType == "不开发票" || this.invoiceType == "增值税普通发票"){
        ...
        accountType(){
            ...
            switch(this.accountType){
                case '公司：三证五证合一':this.photoTypesList = this.allphotoTypesList.slice(0,3);this.card3Show = true;this.card4Show = false;break;
                case '公司：三证五证未合一':this.photoTypesList = this.allphotoTypesList;this.card3Show = true;this.card4Show = true;break;
                case '个体户：已取得税务登记证':this.photoTypesList = this.allphotoTypesList;this.card3Show = true;this.card4Show = true;break;
                case '个体户：未取得税务登记证':this.photoTypesList = this.allphotoTypesList.slice(0,3);this.card3Show = true;this.card4Show = false;break;
                case '个人':this.photoTypesList = this.allphotoTypesList.slice(0,2);this.card3Show = false;this.card4Show = false;break;
            }
        },
...
       })
    ...
    computed:{
        images(){
              var images = [this.file1Idcard,this.file2Businesslicense,this.file4Gtqc,this.file5IdcardBg];
                return images; 
        },
        nowMonth(){
