 ...
 <div class="wrapper">
   <div class="scroll" ref="verifysLeap"></div>
    <div class="protocol"><serProtocol :cid="this.ccid" :flag='1' @close="closeProtocol" v-show="showProtocol"></serProtocol></div>
    <div class="verify" ><ser-verifys :cid="this.ccid"  :showBtn="showBtn" @close="close" v-show="showVerify" @updatePage="updatePage"></ser-verifys></div>
    <!-- 搜索栏和过滤器 -->
    <ser-search 
    :list='this.showlist' 
    :flag='true'
   ...
   <xieyiblock v-show="showBlock" @hiddenBlock="hiddenBlock()"  :ccid='this.ccid' :ccName = "this.ccName" :showReject="showReject" @updatePage="updatePage"></xieyiblock>
    <!-- 表格展示区 -->
    ...
    element-loading-spinner="el-icon-loading"
     >
          ...
          <el-table-column label="联系人/电话" width="130" > 
            <template slot-scope="scope" >
              <p  style="font-size:14px"><span v-show="!scope.row.WL_AGENT_NAME">[暂无]</span><span v-show="scope.row.WL_AGENT_NAME">[{{scope.row.WL_AGENT_NAME}}]</span></p>
              <p  style="font-size:14px"><span v-show="!scope.row.JURIDIC_PERSON_HANDSET">[暂无]</span><span v-show="scope.row.JURIDIC_PERSON_HANDSET">[{{scope.row.JURIDIC_PERSON_HANDSET}}]</span></p>
            </template>
          </el-table-column>
          ...
          <template slot-scope="scope">
             <el-button type="text" size="large" style="color:blue" @click="handleVerify(scope.row)" 
             v-show="
          ...
          <template slot-scope="scope">
            <el-button type="text" size="large" style="color:blue" @click="createCer(scope.row.ACCOUNT1_NAME,scope.row.CID,$event)" 
            v-show=" scope.row.STATE == 'APPROVED'&& scope.row.YLCSTATE != 'SALEMANMODIFYING'&&scope.row.YLCSTATE=='SALEMANFILLING'&&(position =='SALEMAN_M'||position == 'SALEMAN_S') ">创建</el-button>
             
             <el-button type="text" size="large" style="color:red" @click="createCer2(scope.row.ACCOUNT1_NAME,scope.row.CID,$event)" 
             v-show="scope.row.YLCSTATE == 'SALEMANMODIFYING'&&(position =='SALEMAN_M'||position == 'SALEMAN_S')">修改</el-button>
         
            <el-button type="text" size="large" style="color:#85ca80" @click="queryProtocol(scope.row)" 
             v-show="(scope.row.YLCSTATE == 'CUSTOMERAFFIRM'||scope.row.YLCSTATE == 'ASM_CHECKING'||scope.row.YLCSTATE == 'DEP_MARKET_CHECK'||scope.row.YLCSTATE == 'CSA_CHECK')&&(position =='SALEMAN_M'||position == 'SALEMAN_S')">查看</el-button> 
          
          </template>
          </el-table-column>
          <el-table-column  label="查看" width="60" >
          <template slot-scope="scope">
             <el-button type="text" size="large" style="color:#85ca80" @click="handleQuery(scope.row)" 
...
export default {
  ...
  methods: {
    ...
    rowClick(row,column){//点击每一行将信息列出来
        console.log(row);
    },
    handleVerify(row){
      ...
      this.$refs.verifysLeap.scrollIntoView();
       console.log(row);
        //position --- user的岗位
        var position = this.$store.state.user.pos[0].position;
        console.log('当前权限等级是',position)
        console.log('当前行的资料卡状态是',row.STATE);
        
       if(
          (row.STATE == "BUSINESSCHECKING" && (position == 'SALEMAN_M'|| position == 'SALEMAN_S'))|| //业务员审核中，业务员
          (row.STATE == "BIILDEPCHECKING" && (position == 'BILLDEP_APPROVE'))||//订单部审核中，订单部
          (row.STATE == "APPROVED" &&  (position == 'SALEMAN_M'|| position == 'SALEMAN_S'||position =='VSMAPPROVEXII'||position == 'MARKETCHECKER'))//已通过，业务员
      ){ //只有在资料卡状态是业务员审核中的时候才能审核资料卡\
           console.log("当前选中行客户id"+row.CID)
            this.ccid = row.CID;  //正确账号
            if(row.STATE == 'APPROVED'||position == 'VSMAPPROVEXII'||position =='MARKETCHECKER')  this.showBtn = false;//如果是已通过账号，就不用展示确定退回按钮
  ...
  mounted() {
    console.log("当前登录的人是"+this.$store.state.user.data.realName+'权限是'+this.$store.state.user.pos[0].position);
     this.loading = true;
    this.$axios
      .post("/yulan/customerInfo/getNcustomerinfo.do", {
        page: "1",
